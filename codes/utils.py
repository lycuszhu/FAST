...
def _nested_to_2d_with_pad(X_train_nested, X_test_nested):
    """
    Convert nested DataFrames to dense (n,Lmax) arrays by concatenating columns per row
    and padding with zeros to a *global* Lmax computed over train ∪ test.
    """
    import pandas as pd
    ...
    for i in range(len(X_train_nested)):
        L = 0
        for j in range(X_train_nested.shape[1]):
            L += _concat_len(X_train_nested.iat[i, j])
        train_lengths.append(L)
    test_lengths = []
    for i in range(len(X_test_nested)):
        L = 0
        for j in range(X_test_nested.shape[1]):
            L += _concat_len(X_test_nested.iat[i, j])
...
def load_ucr_uea_sktime(
    ...
    Robust UCR/UEA loader:
      - fetch official train/test splits (nested)
      - convert to dense by concatenating columns per row and padding to global Lmax
      - (optional) merge & resplit at the same original fraction
...
def _compute_C_blocks(x: np.ndarray, K: int, d: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    so that for a +2/-1 kernel with s selected taps we can do:
       C = Ca + Cg[i1] + Cg[i2] + ... (+ optional max-pool outside)
    """
    L = x.shape[0]
    span = (K - 1) * d + 1
    out_len = L - span + 1
    ...
    # NOTE: use plain range() here to avoid parfors index overwrite issues when called from a prange region
    for t in range(out_len):
        acc = 0.0
        base = t
        # accumulate background (-1 * x[pos]) and per-tap +3 deltas
        for k in range(K):
            v = x[base]
            acc += v                  # contributes to Ca (background sum)
            Cg[k, t] = 3.0 * v        # +3 correction when this tap is selected
            base += d
        Ca[t] = -acc                  # background is -sum(x[pos]) over span
    return Ca, Cg
        ...
        for j in range(m):
            ...
            for t in range(out_len):
                ...
                if v > thr:
                    cnt += 1
                    sum_idx += t
                    # corrected MPV aggregator: (v - thr)
                    sum_above += (v - thr)
            ...
            out[i, base + 0] = (cnt / out_len) if out_len > 0 else 0.0         # PPV
            out[i, base + 1] = float(max_run)                                   # LSPV
            out[i, base + 2] = (sum_above / cnt) if cnt > 0 else 0.0            # MPV
            out[i, base + 3] = (sum_idx / cnt) / (out_len - 1) if (cnt > 0 and out_len > 1) else -1.0  # MIPV
            if include_max_feature:
                out[i, base + 4] = maxC
    return out
...
def _combinations(n: int, k: int) -> np.ndarray:
    # simple, deterministic combs in lexicographic order
...
class ShapeletSpec:
    ...
    thr_diff: np.ndarray   # biases on diff
    best_eta2: float
...
def lds_quantiles(m: int, offset: float = 0.0) -> np.ndarray:
    ...
    # latin / low-discrepancy-like uniform quantiles in (0,1)
...
def anova_F_eta2_by_column(X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """
    Vectorized one-way ANOVA eta^2 for each column of X (n,m) against labels y (0..C-1).
    Returns: (F, eta2) per column.
    """
    ...
    # classic F (not used downstream for gating, but returned for completeness)
    df_between = C - 1
    df_within = n - C
...
# ---------------------------
# MultiROCKET-like dilation fitting (pure Python)
# ---------------------------
...
def _mr_fit_dilations(input_length: int, num_thresholds_per_kernel: int, max_dilations_per_kernel: int) -> Tuple[np.ndarray, np.ndarray]:
    """
    Return (unique_dils, counts) for K=9 family.
    We sample on log2 grid between 1 and (L-1)/(9-1), collapse duplicates, and allocate counts
    to sum exactly to num_thresholds_per_kernel.
    """
    ...
    max_exp = float(np.log2(max(1.0, (input_length - 1) / 8.0)))  # 9->(9-1)=8
    raw = np.power(2.0, np.linspace(0.0, max_exp, true_max)).astype(np.int32)
    if raw.size == 0:
        return np.zeros(0, dtype=np.int32), np.zeros(0, dtype=np.int32)

    # merge duplicates by RLE
    dils_list: List[int] = []
...
# ---------------------------
# Step-1: Build kernel features (K9 + optional K6/K3)
# ---------------------------

def _make_kernel_values(K: int, select_idx: np.ndarray) -> np.ndarray:
    """
    Build +2/-1 DC-balanced kernel of length K with +2 on select_idx and -1 elsewhere.
    """
    w = -np.ones(K, dtype=np.float32)
    for j in range(select_idx.shape[0]):
        w[int(select_idx[j])] = 2.0
    # mean 0 already (e.g., K=9 with 3x +2 vs 6x -1)
    return w
...
def build_parent_argmax_map(
    ...
    """
    Compute and cache t* (argmax positions) for each ParentKey across all training series.
    Groups work by (family, dilation) to reuse C-construction and vectorize over examples.
    """
    X2d = np.asarray(X_train, dtype=np.float32)
    # group: (fam_name, d) -> list of kernel ids
...
def build_kernel_features_step1(
    *,
    ...
    include_max_feature: bool,         # MR 4 stats; caller may inflate budget externally if True
    ...
    Stage-1:
      ...
      - Rank parents by best eta^2 across emitted stats (both channels)
    """
    ...
    for fam_name in fams:
        ...
        for d_idx in range(dils.shape[0]):
            ...
            # per-kernel thresholds calibrated on RAW probe series
            ...
            # per-kernel thresholds calibrated on DIFF probe series
            x_probe_diff = Xtr_diff2d[probe_idx]  # (L-1,)
            ...
            for kid in range(indices.shape[0]):
                ...
                # raw channel (use RAW thresholds)
                ...
                # diff channel (use DIFF thresholds)
                ...
                Xtr_block = np.hstack([Xtr_block_raw, Xtr_block_diff]) if Xtr_block_raw.size and Xtr_block_diff.size else \
                            (Xtr_block_raw if Xtr_block_diff.size == 0 else Xtr_block_diff)
                Xte_block = np.hstack([Xte_block_raw, Xte_block_diff]) if Xte_block_raw.size and Xte_block_diff.size else \
                            (Xte_block_raw if Xte_block_diff.size == 0 else Xte_block_diff)
...
# ---------------------------
# Step-2: Harvest shapelets (feature-map induced)
# ---------------------------
...
def harvest_shapelets(
    *,
    ...
    """
    Parallel-friendly harvesting:
      - uses cached argmax positions per (parent, series)
      - EB on a subsample (20% default) to decide drop/keep
      - full features via Numba-parallel batch kernel
      - counts budget in *columns* (biases * stats) for base+diff
    """
    Xtr = np.asarray(X_train, dtype=np.float32)
    ytr = np.asarray(y_train, dtype=np.int64)
    n_tr, L = Xtr.shape

    # classes count for shapelet cap
    ...
    for pk in ranked_parents:
        ...
        # span = dilated kernel/shapelet length
        span = (fam.K - 1) * d + 1
        if span > L // 2:
            continue  # shapelet length constraint

        # per-kernel dilation gate
        kern_key = (fam.name, kid)
        best_eta_seen = best_eta_by_kernelid.get(kern_key, -1.0)
        if keep_best_dilation_only and parent_best_eta2.get(pk, 0.0) + 1e-9 < best_eta_seen:
            continue

        # argmax per series, then propose a shapelet per series
        ...
        # For each training series, test candidate window (EB + full)
        for i in range(n_tr):
            t_star = int(tstars[i])
            ...
            # Column budget this shapelet would add (base + diff)
